...
class analisaTopsis extends Controller
{
    ...
    public function get_linguistik()
    {
        ...
        foreach ($mahasiswa as $key) {
            # code...
            if (($key->bahasa_inggris > 0) and ($key->bahasa_inggris <= 30)) {
                # code...
                $key->l_bahasa_inggris = 0.2;
            }elseif (($key->bahasa_inggris > 30) and ($key->bahasa_inggris <= 60)) {
                # code...
                $key->l_bahasa_inggris = 0.5;
            }elseif (($key->bahasa_inggris > 60) and ($key->bahasa_inggris <= 80)) {
                # code...
                $key->l_bahasa_inggris = 0.75;
            }elseif (($key->bahasa_inggris > 80) and ($key->bahasa_inggris <= 100)) {
                # code...
                $key->l_bahasa_inggris= 1;
            }
        }
    ...
    public function get_normalized()
    {
        ...
        foreach ($mahasiswa as $key) {
            ...
            $temp_bahasa_inggris += $key->l_bahasa_inggris*$key->l_bahasa_inggris;
        ...
        foreach ($mahasiswa as $key) {
            ...
            $key->r_bahasa_inggris = $key->l_bahasa_inggris/(sqrt($temp_bahasa_inggris));
    ...
    public function get_terbobot()
    {
        ...
        foreach ($mahasiswa as $key) {
            # code...
            $key->v_prestasi = $key->r_prestasi*$c1->weight;
            $key->v_karya_ilmiah = $key->r_karya_ilmiah*$c2->weight;
            $key->v_bahasa_inggris = $key->r_bahasa_inggris*$c3->weight;
            $key->v_ipk = $key->r_ipk*$c4->weight;
    ...
    public function get_ideal()
    {
        ...
        foreach ($mahasiswa as $key) {
            ...
            $temp_bahasa_inggris[] = $key->v_bahasa_inggris;
            $temp_ipk[] = $key->v_ipk;
        ...
        $solusi = array(
            ...
            'c3' => array('positif' => (!$c3->is_cost) ?  max($temp_bahasa_inggris) :  min($temp_bahasa_inggris),'negatif' => ($c3->is_cost) ?  max($temp_bahasa_inggris) :  min($temp_bahasa_inggris)),
    ...
    public function get_positif_distance()
    {
        ...
        foreach ($mahasiswa as $key) {
            ...
            $key->a_bahasa_inggris = pow(($key->v_bahasa_inggris - $solusi_ideal['c3']['positif']),2);
            $key->a_ipk = pow(($key->v_ipk - $solusi_ideal['c4']['positif']),2);
            $key->a_indeks_sks = pow(($key->v_indeks_sks - $solusi_ideal['c5']['positif']),2);
            $key->a_total = sqrt($key->a_prestasi+$key->a_karya_ilmiah+$key->a_bahasa_inggris+$key->a_ipk+$key->a_indeks_sks);
        }
        return $mahasiswa;
    }
    public function get_negatif_distance()
    {
        ...
        foreach ($mahasiswa as $key) {
            ...
            $key->b_bahasa_inggris = pow(($key->v_bahasa_inggris - $solusi_ideal['c3']['negatif']),2);
            $key->b_ipk = pow(($key->v_ipk - $solusi_ideal['c4']['negatif']),2);
            $key->b_indeks_sks = pow(($key->v_indeks_sks - $solusi_ideal['c5']['negatif']),2);
            $key->b_total = sqrt($key->b_prestasi+$key->b_karya_ilmiah+$key->b_bahasa_inggris+$key->b_ipk+$key->b_indeks_sks);
        }
        return $mahasiswa;
    }
    ...
    public function variabel_linguistik()
    {
        # code...
        $mahasiswa = $this->get_linguistik();
        return Datatables::of($mahasiswa)
                ...
                })->editColumn('l_bahasa_inggris',function($mahasiswa){
                    if ($mahasiswa->l_bahasa_inggris == 0.2 ) {
                        # code...
                        return "Rendah";
                    }elseif ($mahasiswa->l_bahasa_inggris == 0.5) {
                        # code...
                        return "Cukup";
                    }elseif ($mahasiswa->l_bahasa_inggris == 0.75) {
                        # code...
                        return "Tinggi";
                    }elseif ($mahasiswa->l_bahasa_inggris == 1) {
                        # code...
                        return "Sangat Tinggi";
                    }
    ...
    public function bilangan_fuzzy()
    {
        # code...
        $mahasiswa = $this->get_linguistik();
        return Datatables::of($mahasiswa)
    ...
    public function matriks_keputusan_ternormalisasi()
    {
        # code...
        $mahasiswa = $this->get_normalized();
        return Datatables::of($mahasiswa)
    ...
    public function matriks_keputusan_terbobot()
    {
        # code...
        $mahasiswa = $this->get_terbobot();
        return Datatables::of($mahasiswa)
    ...
    public function matriks_solusi_ideal()
    {
        # code...
        $data['solusi'] = $this->get_ideal();
        return view('admin.topsis.matriks_solusi_ideal',$data);
    }
